...
from board_util import GoBoardUtil, BLACK, WHITE, EMPTY, BORDER, PASS, \
                       MAXSIZE, coord_to_point, INFINITY
import numpy as np
import re
...
class GtpConnection():
    ...
    def genmove_cmd(self, args):
        ...
        result = self.solver()
        if (len(result) > 1):
            color = color_to_int(result[0])
            coord = move_to_coord(result[1], self.board.size)
            move = coord_to_point(coord[0],coord[1], self.board.size)
    ...
    def solve_cmd(self, args):
       result = self.solver()
       self.respond("{}\n".format(result[0])) if len(result) == 1 else self.respond("{} {}".format(result[0],result[1]))
